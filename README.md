...
-   [앱 구현 과정 및 트러블 슈팅](#앱-구현-과정-및-트러블-슈팅)
    ...
    -   [접근성 적용](#접근성-적용)
        -   [해당 부분에서 했던 고민점들](#해당-부분에서-했던-고민점들-4)
...
|                             고민                             |
| :----------------------------------------------------------: |
| **CreateML**과 **Keras**를 사용해서 모델을 각각 만들었는데, 파일 용량이 서로 차이가 나는 이유가 무엇일까? |

아래 이미지에서 볼 수 있듯이 **CreateML**로 학습한 파일은 50KB로 비교적 작은 반면, **Keras**로 학습한 파일은 4.8MB로 용량이 비교적 컸습니다.
...
`UIStackView`를 사용하여 버튼들을 배치할 때, `addSubview(_:)`를 사용했었는데, 아래 이미지처럼 원하는 대로 배치가 되지 않고 겹쳐서 배치가 되는 것을 확인했습니다.
...
하지만 코드 리뷰어가  `fatalError` 같은 경우 앱이 죽는 경우가 발생하게 되는데, 그러면 사용자는 어떤 에러 때문에 앱이 종료되는지 알지 못할 뿐더러 사용에 있어서 매우 중요한 것이라서 의도적으로 앱을 종료하도록 하는 것이 아니라면 좋은 예외처리가 아니라는 의견을 줬습니다. 그래서 어떤 처리 방법이 있을지 고민했는데,

1. `UIAlertController` 를 통한 알림
2. 기존의 `UILabel` 을 통한 알림

이렇게 두 가지 방법을 생각했습니다. 사용자들에게 어떤 문제인지를 알려주는 것이 주 목적인데 그 것을 위해서 새로운 UI 컴포넌트를 생성하는 것 보다는, 기존에 있던 UI 컴포넌트를 재사용하여 사용자에게 에러 메시지를 전달하는 것**(2번 방법)**이 더 나은 방법이라 판단했습니다. 그리고 추가로, 1회 실패 후 메시지를 출력하는 것보다, 3회 재시도를 한 후, 메시지를 출력하는 것이 자연스럽고 사용성 측면에서도 좋은 방법이라고 판단했습니다. 따라서 매개변수에 있는 `retry` 값 만큼 재귀 호출을 하는 `dispatchWork` 메서드를 구현했습니다.
...
다이나믹 타입을 적용하기 위해서 각 폰트 크기에 맞는 텍스트 스타일로 구현했습니다. 구현 후, ***Accessibility Inspector***를 통해 접근성 검사를 완료했습니다. 왼쪽 이미지는 기본으로 적용되어 있는 `UIContentSizeCategory.large`  사이즈일 때이며, 오른쪽 이미지는 `UIContentSizeCategory.accessibilityExtraExtraExtraLarge` 사이즈 일 때 입니다. 최대 사이즈일 때도 글자가 잘리거나 프레임을 벗어나지 않습니다.
...
기존에는 모든 레이블과 버튼에 `adjustsFontForContentSizeCategory` 값과 `preferredFont(forTextStyle:)` 메서드를 각각 사용해줬습니다. 하지만 `extension`을 통해 메서드로 확장하면 더 좋은 방법일 것이라고 생각했습니다.
